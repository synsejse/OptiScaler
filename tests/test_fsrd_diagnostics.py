...
class Diagnostics(unittest.TestCase):
    ...
    def test_reset_requests_same_frame_capture_on_render_thread(self):
        ...
        self.assertIn("_resetDenoiserHistory.load() && FSRDResearch::Request(Handle()->Id)", request)
        ...
        self.assertIn("_lastDiagnosticResetFrame.store(_frameCount)", evaluate)
    ...
    def test_controls_are_session_only_thread_safe_requests(self):
        for field in ("_identityDenoiser", "_resetDenoiserHistory"):
            self.assertIn(f"std::atomic<bool> {field} {{ false }}", HEADER)
        ...
        self.assertIn("currentFeature->GetUpscalerType() == Upscaler::FSRD", menu)
